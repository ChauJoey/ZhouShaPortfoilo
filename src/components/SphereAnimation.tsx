...
export default function SphereAnimation() {
  ...
  useEffect(() => {
    const sphereEl = wrapperRef.current?.querySelector(
      ".sphere-animation"
    ) as HTMLElement | null;
    if (!sphereEl) return;

    const spherePathEls = sphereEl.querySelectorAll<SVGPathElement>(
      ".sphere path"
    );
  ...
  return (
    <div ref={wrapperRef} className="animation-wrapper">
